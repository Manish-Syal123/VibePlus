...
type FileCollection = { [path: string]: string }; // key value pair{} -> { path(string):content(string), "/app/index.tsx": "let a=23", .... etc. }
...
const FileBreadcrumb = ({ filePath }: FileBreadcrumbProps) => {
  ...
  const renderBreadcrumbItems = () => {
    if (pathSegments.length <= maxSegments) {
      ...
      const firstSegemnt = pathSegments[0];
      const lastSegment = pathSegments[pathSegments.length - 1];

      return (
        <>
          <BreadcrumbItem>
            <span className="text-muted-foreground">{firstSegemnt}</span>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbEllipsis />
            </BreadcrumbItem>
...
export const FileExplorer = ({ files }: FileExplorerProps) => {
  ...
  return (
    <ResizablePanelGroup direction="horizontal">
      <ResizablePanel defaultSize={30} minSize={25} className="bg-sidebar">
      ...
      <ResizablePanel defaultSize={70} minSize={50}>
        {selectedFile && files[selectedFile] ? (
          <div className="h-full w-full flex flex-col">
            <div className="border-b bg-sidebar px-4 py-0.5 flex justify-baseline items-center gap-x-2">
